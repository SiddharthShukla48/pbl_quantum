...
def build_qubo(adjacency, K, lambda1=10000, lambda2=5000):
    ...
    # Constraint 1: Each exam gets exactly one color
    print("\nAdding constraint: Each exam one color...")
    for exam in range(n):
        for c in range(K):
            idx = var_idx(exam, c)
    ...
    # Constraint 2: Adjacent exams different colors
    print("Adding constraint: No conflicts in same slot...")
    ...
    print(f"✓ Added {num_conflict_pairs} conflict pairs")
    print(f"✓ QUBO built: {np.count_nonzero(Q)} non-zero entries")
...
def main():
    ...
    # Step 2: Build QUBO
    Q = build_qubo(adjacency, K=args.k)
